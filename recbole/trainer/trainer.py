...
from recbole.trainer.optimize import get_constant_schedule_with_warmup, get_linear_schedule_with_warmup
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        #self.loss_update = config['loss_update']
        ...
        if config['schedule'] == 'constant':
            num_training_steps = self.epochs * (config['inters_num']/config['train_batch_size'])
            self.schedule = get_constant_schedule_with_warmup(self.optimizer,num_warmup_steps=int(num_training_steps*config['warmup_rate']))
        elif config['schedule'] == 'linear':
            num_training_steps = self.epochs * (config['inters_num']/config['train_batch_size'])
            self.schedule = get_linear_schedule_with_warmup(self.optimizer,num_warmup_steps=int(num_training_steps*config['warmup_rate']), num_training_steps=num_training_steps, min_value=config['min_lr'])
        elif config['schedule'] == 'None':
            self.schedule = None
        else:
            raise NotImplementedError("only support schedule mode: [constant, linear, None]")
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        #print("current_lr: ",self.learning_rate)
        #print("current_IPS clip: ",self.model.loss.clip)
        return total_loss
