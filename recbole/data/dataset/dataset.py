...
class Dataset(object):
    ...
    def popularity_of_users(self):
        if isinstance(self.inter_feat, pd.DataFrame):
            raise NotImplementedError
        else:

            #users = np.sort(self.inter_feat[self.uid_field].numpy())

            #print(users[:10])
            #sorted(self.inter_feat[self.uid_field].numpy(),key=lambda x:x[1],reverse=True)
            users = np.sort(self.inter_feat[self.uid_field].numpy())
            value = list(Counter(users).values())
            max_value = np.max(value)

            return {i:j/max_value for i,j in Counter(users).items()}

    def popularity_of_items(self):
        print(len(self.inter_feat))
        if isinstance(self.inter_feat, pd.DataFrame):
            raise NotImplementedError
        else:

            items = np.sort(self.inter_feat[self.iid_field].numpy())
            value = list(Counter(items).values())
            max_value = np.max(value)

            return {i:j/max_value for i,j in Counter(items).items()}
    ...
    def build(self):
        ...
        if split_mode == 'RS':
            ...
            if group_by is None or group_by.lower() == 'none':
                ...
                datasets = self.split_by_ratio(split_args['RS'], group_by=self.iid_field)
            else:
