...
class GRU4Rec(SequentialRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            pos_items_emb = self.item_embedding(pos_items)
            pos_score = torch.sigmoid(torch.sum(seq_output * pos_items_emb, dim=-1, keepdim=False))  # [B]
            loss = self.loss_fct(pos_score,label)
            return loss
    ...
    def predict(self, interaction):
        ...
        scores = torch.sigmoid(torch.mul(seq_output, test_item_emb).sum(dim=1))  # [B]
    ...
    def full_sort_predict(self, interaction):
        ...
        scores = torch.sigmoid(torch.matmul(seq_output, test_items_emb.transpose(0, 1)))  # [B, n_items]
        return scores
